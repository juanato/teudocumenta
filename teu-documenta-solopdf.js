 ...
 * fichero TOPICOS.TXT
 * 
 * En la misma carpeta se descargan los PDF, se genera un HTML con enlace a ellos y otro usando GOOGLE DOCS para verlos ONLINE
 * 
 * 
...
const cVersion = "TEU DOCUMENTA 2.0 SOLOPDF";
...
function CargaGL(cBusca, cDonde, nCuantos)
{
 ...
 // iimSet(  "numero", nLineas ); //     
  iimSet(  "paginado", nCuantos );   // Resultados de la búsqueda por paginado de 10 en 10
    ...
    var cMacro = "CODE:";
// Versión que solo busca PDFs en GOOGLE    
...
if (cUrl == "vacío" )    
   {
    HuboError("No se encuentran en esa posición TEXTO RECONOCIBLE. Se va a obviar este enlace. CLASS st");
...
else
   {
    ...
    if (cUrl == "vacío" )    
       {      
       HuboError("No se encuentra un enlace a PDF. Se va a obviar este enlace. CLASS_ Rm");      
       iimSet(  "linea", nLinea.toString() ); // entrada de resultados de gOOgle
...
cLinea =  '<h3>Generador de DICCIONARIO DE DATOS de TRAZAS</h3>'
aEnlaces.push(cLinea); 
...
function ExistePath( oPath, cpath)
{

 
  var trypath = oPath.clone();
  trypath.appendRelativePath( cPath)
 ...
 */
function Barra(percent, speed, color)
 {
  color = color || 'green';
  ...
  // print to stdout
  Enviaww( BAR.substr(0, rounded).bold[color] +
                       NOBAR.substr(0, 50 - rounded) + percent + '%' + ' ' + speed + '  \r');
}
...
function Enviaww( cTexto )
{
  

             iimDisplay( "Enviando texto página web "+ cTexto ); // Contador de página de ANUNCIOS
             iimSet(  "texto", cTexto ); 
             cMacro2 =IniciaMacro();
             cMacro2 +="URL GOTO=javascript:window.write({{texto}})"+"\n";
             iimPlay(cMacro2);  
             HuboError('Enviado texto a la página' );   
