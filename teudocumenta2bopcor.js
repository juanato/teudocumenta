 ...
 * Gestor Documental que extrae via GOOGLE PDF
...
var lSalir = false; // Rompemos los bucles
...
cTeuFichero = gTiempo()+DameUnico();  //Nombre de la carpeta
cTeuFichero = replaceAll( cTeuFichero, " ", "_" );

HuboError("Se va a ALMACENAR todo lo encontrado en la carpeta " + cTeuFichero );
CreaSubDir(cTeuFichero);    // Crea la carpeta temporal

//GrabaFic( cFile, cTeuFichero )
GrabaFic( cTeuFichero+imns.FIO.psep+cBuscar, cVersion );
GrabaFic( cTeuFichero+imns.FIO.psep+cBuscar, cBuscar );
GrabaFic( cTeuFichero+imns.FIO.psep+cBuscar, cBoletin );
HuboError(cBuscar+" en "+cBoletin +" " + cBuscar+" " + cTeuFichero);
Texto2Head(cBuscar+" en "+cBoletin, cBuscar+".html", cTeuFichero);
var cDesde = getiMacrosFolder("Macros")+DetectaBS()+ "pdfobject.js" ;
var cPara = cVia = getiMacrosFolder("Downloads")+DetectaBS()+cTeuFichero+imns.FIO.psep+"pdfobject.js";
...
if (cBoletin.indexOf("dipucordoba")  == -1 ) 
{
}
else

{
 // Tratamiento del BOP de Córdoba para los HTML


 
  
  CargaBOPCOR( cBuscar );
  BOPCORDescarga( cBuscar, cTeuFichero );     
...
// {  
  ...
  VuelcaTxt();    // Gestión de los PDF convertirlos a TXT

    
    
 Texto2Fin( cBuscar+".html", cTeuFichero ); // Finaliza el HTML 
...
//cTeuFichero+imns.FIO.psep+cBuscar
    ...
    if (cUrl == "vacío" )    
       {      
        ...
        { 
         ...
         Texto2Body(cDesLink, cLink, cBuscar+".html", cTeuFichero) ;         
...
function gTiempo()
{
 ...
 var cTeuFichero = nAno.toString()+cMes+cDia;
 return  cTeuFichero ;
...
aEnlaces.push(cLinea);
cLinea = "function LeeTxt(cTxt){";
aEnlaces.push(cLinea);
cLinea = "window.location.href = cTxt;";
aEnlaces.push(cLinea);
//cLinea = "sleep(5);";
//aEnlaces.push(cLinea);
...
aEnlaces.push(cLinea);
cLinea =  '<h2>PATRÓN DE BÚSQUEDA UTILIZADO: '+ cTitulo + "</h2>"    ;
aEnlaces.push(cLinea); 
cLinea =  '<h2>'+ cVersion + "</h2>"    ;
aEnlaces.push(cLinea); 
// var cVia = getiMacrosFolder("Downloads")+DetectaBS()+cFile;
cLinea =  '<h3>Generador de DICCIONARIO DE DATOS de TRAZAS presentes BOLETINES ELECTRÓNICOS</h3>'
aEnlaces.push(cLinea); 
cLinea =  '<h3>Gestión DOCUMENTAL de TRAZAS en Boletines OFICIALES (PDF-HTML-TXT)</h3>'
aEnlaces.push(cLinea); 
cLinea =  '<h3>Trazabilidad de DERECHOS Y OBLIGACIONES EN FUENTES DE ACCESO AL PÚBLICO</h3>'
aEnlaces.push(cLinea); 
cLinea =  '<h4>conforme Ley Orgánica LOPD art. 3j 15/1999</h4>'
aEnlaces.push(cLinea); 
...
aEnlaces.push(cLinea);      
cLinea = '<a href =' + 'javascript:LeeTxt("' + cNombretxt +  '");'+' style="color:coral'+ '">'+cTitulo+"</a>"  ;
...
function Texto2Body1(cTitulo, cLink, cFichero, cVia)
{
...
/*
VERSION PARA HTML EXCLUSIVAMENTE
*/
...
cNombretxt = cLink.substring(cLink.lastIndexOf("/") + 1 );
cNombretxt = cNombretxt.substring(0, cNombretxt.lastIndexOf(".") )+".txt"; 
HuboError("Se va a grabar la entrada directa de URL con HTML " + cTitulo + " apuntando a " + cLink  );   
cLinea = '  <br><br> ' ;      
aEnlaces.push(cLinea);
  
cLinea = '<a href =' + cLink + "/full" + " style=color:green> "+cTitulo+"</a>"  ;
cLinea = '<h1>'+ cLinea + "</h1>"    ;
aEnlaces.push(cLinea);      
cLinea = '<br>Ver URL ONLINE y BUSCAR patrón <br>' ;   ;
aEnlaces.push(cLinea);      
cLinea = '<a href =' + 'javascript:LeeTxt("' +  cLink+ "/full"  +  '");'+' style="color:red'+ '">'+cTitulo+"</a>"  ;
aEnlaces.push(cLinea);
cLinea = " <br>" ;
aEnlaces.push(cLinea);
cLinea = " <br>" ;
...
//Texto2Head(cBuscar+cBoletin, cBuscar+".html", cTeuFichero);
}                              
...
{                // Ojo que ejecutamos en /Macros
        var cVia, cRunOS, cCmdLine, cTmpFile, oFile, cSufijo ;    
          ...
          cVia =  getiMacrosFolder("Macros")+imns.FIO.psep;
          cSufijo = ".sh";
          if ( DameOS() == "WINNT" )
           { cRunOS = "call ";
             cSufijo = ".cmd"; }
          ...
          if ( DameOS() == "Darwin" )
           ...
           cCmdLine = cCmdLine + imns.str.trim(cRunOS + " " + cVia +imns.FIO.psep+"poppler-0.33.0"+imns.FIO.psep+"bin"+imns.FIO.psep+'pdftotext.exe'+ " " +  cFile1);            
           cCmdLine = cCmdLine +  DetectaCR();        


           cTmpFile = "corre"+DameUnico()+cSufijo;
            //window.alert( cVia +cTmpFile );
            //window.alert( cCmdLine  );
...
/* Descarga 20 entradas del buscador BOPCORDOBA y las almacena     
...
function CargaBOPCOR(cBusca )
{
  //&query=GARI+MUNSURI&search_on=all
  var cMacro = "CODE:";
  cBusca = replaceAll( cBusca, " ", "+");
  iimSet(  "texto", cBusca ); // texto a buscar
  cMacro +="URL GOTO=http://www.dipucordoba.es/bop/index/search?date_down=&date_up=&match_mode=all&page=1&query={{texto}}"+"\n";
  cMacro +="WAIT SECONDS=5"+"\n";
  iimPlay(cMacro);      
  HuboError(   'Se ha cargado BOPCORDOBA con la búsqueda' );   
  return cMacro;      
}
...
function BOPCORDescarga(cNomDes1, cCarpeta1 )
{
 nItem = 1;
do {
    ...
    if (lRompeBucle == true)
       ...
       {
         //cNomDes1 = cNomDes1 + nItem.toString();
        bcDescarga20( nItem, cNomDes1 , cCarpeta1 );
        ++nItem;
...
// Descarga 20 item del BOP CORDOBA

function bcDescarga20( nLinea, cNomDes, cCarpeta )
{
...
cMacro =  IniciaMacro();
cMacro +="TAG POS={{linea}} TYPE=A ATTR=HREF:/bop/show/*  extract=TXT" + "\n";
cMacro +="saveas type=extract folder={{carpeta}} file={{fichero}}html.txt" + "\n";
iimPlay(cMacro);


/*
TAG POS=1 TYPE=A ATTR=HREF:/bop/show/* EXTRACT=TXT
TAG POS=1 TYPE=A ATTR=HREF:/bop/show/* EXTRACT=HTM
TAG POS=1 TYPE=A ATTR=HREF:/bop/show/* EXTRACT=HREF
*/
...
iimSet(  "linea", nLinea.toString() ); // entrada de resultados de gOOgle
cMacro ="CODE:TAG POS={{linea}} TYPE=A ATTR=HREF:/bop/show/*  extract=TXT" + "\n";   
iimPlay(cMacro);
cUrl = DameEx(); 
if (cUrl == "vacío" )    
   {

    HuboError("No se encuentran en esa posición TEXTO describiendo el resultado. Se va a obviar este enlace. BOPCORDOBA texto");
    //lRompeBucle = true;
   }
else
   {
    ...
    cMacro =  IniciaMacro();
    cMacro +="TAG POS={{linea}} TYPE=A  ATTR=HREF:/bop/show/* extract=TXT" + "\n";
    cMacro +="saveas type=extract folder={{carpeta}} file={{fichero}}textbcor.txt" + "\n";
    iimPlay(cMacro);
    cDesLink = cUrl;
    HuboError("Se ha encontrado TEXTO en la entrada bopcordoba nº " + nLinea.toString() + " " +  + " " + cDesLink );
  
    
    iimSet(  "linea", nLinea.toString() ); // entrada de resultados de gOOgle
    cMacro ="CODE:TAG POS={{linea}} TYPE=A  ATTR=HREF:/bop/show/* extract=HREF" + "\n";
    iimPlay(cMacro); 
    cUrl = DameEx();      
    if (cUrl == "vacío" )    
       {    
       //lRompeBucle = true;               
       HuboError("No se encuentran en esa posición ENLACE al descriptor. Se va a obviar este enlace. BOPCORDOBA HREF.");
       }  
   else
     {
      ...
      iimSet(  "carpeta", cVia); // nombre de la carpeta del expediente
      cMacro =  IniciaMacro();
      cMacro +="TAG POS={{linea}} TYPE=A  ATTR=HREF:/bop/show/* extract=HREF" + "\n";
      cMacro +="saveas type=extract folder={{carpeta}} file={{fichero}}html-bopcor-link.txt" + "\n";
      iimPlay(cMacro);
    ...
    if (  CompruebaLink( cUrl) == false )
     { 
      cLink = cUrl; 
      HuboError("Se ha conseguido extraer el link al HTML " + cLink );
      HuboError("Se ha grabado de BOPCORDOBA  entrada HTML nº " + nLinea.toString() + " en " + cNomDes + " en la carpeta " + cCarpeta );
      Texto2Body1(cDesLink, cLink, cBuscar+".html", cTeuFichero) ;       //Versión para HTML ONLINE    
    }
...
Comprueba el enlace detectado, que no se genere por duplicado
*/

function CompruebaLink( cUrlLink)
{

var lRepetido = false;


if ( aRepetidos.indexOf(cUrlLink ) == -1 )

 {         
   // No está repetido aún

   aRepetidos.push(cUrlLink );
 }
