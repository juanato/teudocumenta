...
//cTiempo+imns.FIO.psep+cBuscar
 ...
 var cVia = getiMacrosFolder("Downloads")+imns.FIO.psep+cCarpeta;
...
if (cUrl == "vacío" )    
{
HuboError("No se encuentran coincidencias. Se va a obviar este enlace.");
}
else
{
cDesLink = cUrl;
//TAG POS=1 TYPE=H3 ATTR=CLASS:R extract=href
HuboError("Se ha conseguido extraer el texto del PDF " + nLinea.toString() + " " +  + " " + cDesLink );
...
HuboError("Se ha conseguido extraer el link al PDF " + cLink );
 //EVENT TYPE=CLICK SELECTOR="#rso>DIV>LI:nth-of-type({{!loop}})>DIV>H3>A"        
...
//TAG POS=1 TYPE=H3 ATTR=CLASS:R extract=href
//saveas type=extract folder={{carpeta}} file={{fichero}}-href.txt
//var   cMacro =  IniciaMacro();
...
//TAG POS=2 TYPE=CITE ATTR=CLASS:_Rm extract=TXT
//saveas type=extract folder={{carpeta}} file={{fichero}}.txt
//cMacro +="saveas type=extract folder={{carpeta}} file={{fichero}}-href.txt" + "\n";
//cMacro +="SET !EXTRACT NULL" + "\n";
//iimPlay(cMacro);
//cLink = DameEx();     

//window.prompt(cURL);
//HuboError("Se ha grabado el link de la entrada nº " + nLinea.toString() + " " + cLink);


HuboError("Se ha grabado de GOOGLE entrada nº " + nLinea.toString() + "en " + cNomDes + " en la carpeta " + cCarpeta );      
//Texto2Body(cTitulo, cLink, cFichero, cVia)                      
...
const cDocs = "http://docs.google.com/gview?url=";
const cDocs2      = "#search/" + cBuscar;
var cNombreLocal;
